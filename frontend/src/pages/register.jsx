...
function RegisterPage({ onBackToLogin }) {
	...
	function handleSubmit(event) {
		...
		setStatusMessage(
			`Account created as ${selectedRole}. Access granted to ${roleAccess[selectedRole].join(', ')}.`,
		)
	}
			...
			<div className="relative mx-auto flex w-full max-w-7xl flex-col gap-6">
				...
				<main className="grid gap-6 lg:grid-cols-[1.08fr_0.92fr]">
					<section className="relative overflow-hidden rounded-[2rem] border border-white/10 bg-slate-950 p-7 text-white shadow-[0_32px_90px_rgba(15,23,42,0.2)] sm:p-9">
						...
						<div className="absolute -right-12 top-10 h-40 w-40 rounded-full border border-white/10 bg-white/5" />
						...
						<div className="relative flex h-full flex-col justify-between gap-10">
							<div className="space-y-5">
								...
								<h2 className="max-w-xl text-4xl font-semibold tracking-tight text-white sm:text-5xl">
									Create a polished account with the right role.
								</h2>
								<p className="max-w-xl text-base leading-7 text-slate-300 sm:text-lg">
									Register as an admin, branch manager, or employee. The sign-up experience stays calm,
									luxurious, and easy to finish.
								</p>
							</div>
					...
					<section className="rounded-[2rem] border border-slate-200/80 bg-white/80 p-6 shadow-[0_28px_80px_rgba(15,23,42,0.12)] backdrop-blur-xl sm:p-8">
						...
						<div className="mt-6 rounded-3xl border border-slate-200 bg-slate-50/90 p-5 shadow-sm">
